...
class eoswap_tester : public tester {
 public:
   eoswap_tester() {
      ...
      create_currency(
          N(roxe.token), config::system_account_name, roxe::chain::asset::from_string("1000000.000000 EOS"));

      issue(config::system_account_name, roxe::chain::asset::from_string("1000000.000000 EOS"));
...
BOOST_FIXTURE_TEST_CASE(collect_tests, eoswap_tester) try {
   ...
   //    BOOST_REQUIRE_EQUAL(roxe::chain::asset::from_string("100.000000 " + token_name), ab);
}
FC_LOG_AND_RETHROW()
...
BOOST_FIXTURE_TEST_CASE(mint_tests, eoswap_tester) try {
   ...
   BOOST_REQUIRE_EQUAL(roxe::chain::asset::from_string("5.000000 " + token_name), ab);
}
FC_LOG_AND_RETHROW()
...
BOOST_FIXTURE_TEST_CASE(extransfer_tests, eoswap_tester) try {
   ...
   BOOST_REQUIRE_EQUAL(roxe::chain::asset::from_string("0.000001 " + token_name), get_balancex(user2, sym));
}
FC_LOG_AND_RETHROW()
