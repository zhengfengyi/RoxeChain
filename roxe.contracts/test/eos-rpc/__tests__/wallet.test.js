...
const { DEFAULT_WALLET_PRIVATE_KEY, PRIVATE_KEY_INITA ,PRIVATE_KEY_2} = require('../config');
...
describe('WALLET - #list_keys', () => {
    it('should list all keys in the wallet', () => {
        ...
        w.create(name)
            .then(res => {
                ...
                w.import_key(name, PRIVATE_KEY_2)
                    .then(res => {
                        expect(res).toBeDefined()
                        expect(res).toEqual({});
                        w.list_keys(name, password)
                            .then(res => {
                                expect(res).toBeDefined()
                                expect(Array.isArray(res)).toEqual(true);
                            })
    ...
    it('should set wallet timeout', () => {
        w.unlock("default", DEFAULT_WALLET_PRIVATE_KEY)
            .then(res => {
                ...
                const scope = ["useraaaaaaab", "useraaaaaaac"];
                const read_scope = [];
...
describe('WALLET - #import_key', () => {
    it('should import a key', () => {
        w.unlock('useraaaaaaaa', PRIVATE_KEY_INITA)
            .then(res => {
                expect(res).toBeDefined()
                expect(res).toBeTruthy();
                expect(res).toEqual({});
                w.import_key('useraaaaaaaa', PRIVATE_KEY_INITA)
                    .then(res => {
                        console.log('RES', res)
                        expect(res).toBeDefined()
                        expect(res).toEqual({});
